...
const GestureInput: React.FC = () => {
  ...
  const isDoubleHandHeart = (landmarksLeft: NormalizedLandmark[], landmarksRight: NormalizedLandmark[]) => {
    const L = landmarksLeft;
    const R = landmarksRight;

    const tipsClose =
      Math.hypot(L[8].x - R[8].x, L[8].y - R[8].y) < 0.05 &&
      Math.hypot(L[4].x - R[4].x, L[4].y - R[4].y) < 0.05;

    const aligned =
      Math.abs(L[8].y - R[8].y) < 0.05 &&
      Math.abs(L[4].y - R[4].y) < 0.05;
    return tipsClose && aligned;
  }
